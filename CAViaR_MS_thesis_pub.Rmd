...
While there are other ways to understand and measure downside risk, a commonly accepted method is using value-at-risk (VaR). The metric is understood as follows: a one-day 1% VaR of -10 million dollars for a portfolio means that the portfolio will lose at least 10 million dollars of its value on the 1% worst trading days. A major advantage of VaR is that it distills a distribution of returns into one number. As such, VaR is often used in stress testing by regulatory agencies in the United States, the United Kingdom, and Europe [@Holton2014]. 

A popular approach to modeling VaR called RiskMetrics [@Longerstaey1996] was introduced by J.P. Morgan in 1994 and re-released in 1996. The model assumed that a "portfolio or any asset's returns follow a normal distribution over time" and used this along with the "variance-covariance method" to calculate VaR [@Investopedia2019]. While this was certainly a step forward at the time, perhaps the model's greatest downfall is the pretense of knowledge that modeling the distribution of returns in entirety is possible. The elegant simplicity of using a normal distribution is appealing - having only to estimate the mean and the variance to get a universal picture of returns is certainly desirable, and perhaps necessary in a time of comparatively limited computing power. 
...
acc_df[-(1:2),] %>% kable(caption = "Accuracy of VaR Forecast for AMZN and PG Over Last 200 Trading Days in 2008", digits = 3) %>% kable_styling(full_width = F) %>% footnote(general = "Tested Using the Symmetric Absolute Value Model")
...
A useful means of predicting stock movements in the future is the Stock and Watson diffusion index. The model is outlined below, which is adapted from $\textit{Multivariate Time Series Analysis With R and Financial Applications}$ by Ruey S. Tsay [@Tsay2014].
...
Per Engle and Manganelli, under the model specified above and using 8 assumptions given below, $\boldsymbol{\hat{\beta}} \overset{p}\rightarrow \boldsymbol{\beta^0}$ where $\boldsymbol{\hat{\beta}}$ is the parameter vector that minimizes the loss function specified above. There are 8 assumptions listed in the paper.
...
For the sake of brevity, the results with only U.S. ETFs, global ETFs, or bonds are not included in this paper. The results are similar to the results below. To test how well the models do at different VaR levels, 1%, 5%, and 10% are tested.
...
### 1% VaR Table - Multivariate Model

```{r, results=  'asis'}
# Call the above function
v1_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2008_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```

### 1% VaR Table - Univariate Model

```{r, results=  'asis'}
# Call the above function
v1_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2008_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```

### 1% VaR - Optimal Parameters for Multivariate Model

```{r, results=  'asis'}
# Call the above function
v1_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2008_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2008_all_etf.csv", 0.05, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```

### 5% VaR Table - Multivariate Model

```{r, results=  'asis'}
# Call the above function
v5_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2008_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```

### 5% VaR Table - Univariate Model

```{r, results=  'asis'}
# Call the above function
v5_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2008_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```

### 5% VaR - Optimal Parameters for Multivariate Model

```{r, results=  'asis'}
# Call the above function
v5_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2008_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2008_all_etf.csv", 0.10, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```

### 10% VaR Table - Multivariate Model

```{r, results=  'asis'}
# Call the above function
v10_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2008_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```

### 10% VaR Table - Univariate Model

```{r, results=  'asis'}
# Call the above function
v10_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2008_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```

### 10% VaR - Optimal Parameters for Multivariate Model

```{r, results=  'asis'}
# Call the above function
v10_2008_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2008_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```

The univariate CAViaR models significantly outperform the multivariate models, particularly at the 1% level. The extreme behavior towards the end of 2008 proved difficult for the multivariate model to pick up.
...
```{r, results=  'asis'}
# Call the above function
v1_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2010_all_etf.csv", 0.01, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v1_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2010_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v1_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2010_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v1_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2010_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2010_all_etf.csv", 0.05, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2010_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2010_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2010_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2010_all_etf.csv", 0.10, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2010_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2010_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2010_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2010_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```

The forecasts from the multivariate models are largely in line with those from the univariate CAViaR model. Generally, it seems like the univariate models do a better job tracking the response variable in the case of a large swing because of the moving average component. Also, while the multivariate models had a rate of VaR breaks that was too high for 2008, the rate of VaR breaks was generally too low for the multivariate models in 2010.
...
```{r, results=  'asis'}
# Call the above function
v1_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2014_all_etf.csv", 0.01, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v1_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2014_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v1_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2014_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v1_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2014_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2014_all_etf.csv", 0.05, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2014_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2014_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2014_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2014_all_etf.csv", 0.10, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2014_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2014_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2014_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2014_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```
...
```{r, results=  'asis'}
# Call the above function
v1_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2016_all_etf.csv", 0.01, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v1_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2016_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v1_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2016_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v1_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_1pc_2016_all_etf.csv", 0.01, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2016_all_etf.csv", 0.05, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2016_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2016_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v5_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_5pc_2016_all_etf.csv", 0.05, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2016_all_etf.csv", 0.10, print_graph = 1, print_mv_table = 0, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2016_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 1, print_uv_table = 0, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2016_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 0, print_uv_table = 1, print_opt_param =0)
```
...
```{r, results=  'asis'}
# Call the above function
v10_2016_alletf = var_input_disp("/Users/stevenmoen/Documents/GitHub/CAViaR_MS_thesis/Data_Export/SPY_all_ETF_runs/","var_10pc_2016_all_etf.csv", 0.10, print_graph = 0, print_mv_table = 0, print_uv_table = 0, print_opt_param = 1)
```

As with 2010 and 2014, the results between model classes are similar. The multivariate models perform quite well with VaR breaks at the 5% and 10% level, and all the results (from both model classes) seem to be very closely clustered together.
...
## Future Work - Changepoint Detection Algorithm

One advantage of the diffusion index model is the ability to detect changepoints in the underlying factors, which can serve as a proxy for economic changes. Below is a method that can be explored in future work:
...
The angle $\theta$ could be plotted over time, and changepoints could be detected using Monte Carlo simulation. The reason for using Monte Carlo simulation is that Principal Component Analysis transformations are non-linear, and so calculating an analytical density from the transformed data is intractable. Moreover, the data fed into the PCA transformation is non-normal, which further supports the notion of using Monte Carlo simulation to establish reasonable estimates of uncertainty for detected changepoints.

```{r}
# cav_simul(c("DIS", "GE", "IBM", "MMM", "XOM"), resp_var = "PG", start_date = "2004-01-01", end_date = "2008-12-31", nval = 250, ntest = 250, low_m = 1, high_m = 5, tau = 0.01, uv_list = uv_cav_list)
# cav_simul = function(symbol_list, resp_var, compl_case = 1, adj_close = 1, resp_adj_close = 1, start_date = "1900-01-01", end_date = Sys.Date(), nval, ntest, tau, low_m = 1, high_m, low_p = 1, high_p, uv_list = NULL, no_run = c(0,0,0,0), na_interp = TRUE, print_mdl = 0, print_mp = 0, lag_pred = 1, rowname = NULL, export_csv = 1, path, filename){
  # Select data parameters, pull the data, and percent change the data
  df = diff_index_df(symbol_list = c("DIS", "GE", "IBM",  "XOM"), resp_var = "SPY", compl_case = 1, adj_close = 1, resp_adj_close = 1, start_date = "2004-01-01", end_date = "2008-12-31", lag_pred = 1)
df
  # Take the percent change of the data
  pc_df = pc_diff_index(df)
```
...
#'
#' @examples
changepoint_detect = function (x, m) {
  # Converts the response variables into a matrix
  if (!is.matrix(x)) 
      x = as.matrix(x)
  # nT is number of t time-steps
...
Early in my academic career, I grew to respect the University of Chicago as a truly special institution because of its unparalleled reputation in the field of Financial Econometrics and Statistics. I knew that when I received an offer to study as a master's student that I could not turn it down. Despite all of the late nights working in the Regenstein library, in the John Crerar library, or in Jones, I can truly say that it has been the time of my life earning a master's degree in Statistics at UChicago. Earning an M.S. degree is not the conclusion of my academic journey; I will be pursuing a Ph.D. in Statistics at the University of Wisconsin-Madison beginning in the fall of 2020. While I will be leaving Chicago this summer, I hope to maintain many of the connections that I have formed as a student. I have learned during this present crisis that an academic community doesn't require proximity, and that is good news. I have also learned during my time here that the community at the University of Chicago is bound together by tireless, talented individuals who believe that above all else, often at great personal cost, that they have a commitment to find and share the truth with the world around them. And so let human life be enriched, as our motto goes.

As with many important steps in milestones in life, I must recognize that I did not reach them alone. I would like to thank my parents for their unwavering support and invaluable proofreading help. I would like to thank all of my family - my sister, my cousins, my aunts, my uncles, and my grandparents - for their commitments to me over the past 27 years. I would like to thank all of my friends - from Houston, from Rice University, from Dallas, from UChicago, and elsewhere for their support and check-ins through this process. I would like to thank my beautiful and brilliant girlfriend who has put up with close to two years of FaceTime calls and infrequent visits. 

I must also thank Dr. Wei Biao Wu for his invaluable advising and mentorship. I would also like to thank Dr. Mei Wang for her guidance along this process as the M.S. advisor. In addition, Dr. Matthew Stephens, Dr. Ruey Tsay, and Dr. Lars Peter Hansen all deserve recognition for their suggestions to make this research possible. From Rice, I would like to thank Dr. James Brown for sharing his passion for economics, Dr. Illya Hicks for hiring me for my first research job, and Dr. Russell Green for giving me my first taste of economics research. None of this would be possible without you.
