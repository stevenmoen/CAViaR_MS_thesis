...
mod_di_wl = function (y, x, orig, m, tau, ar_tf = 1, p = 1, print = 0, model = 1) 
{
  ...
  y1 = y[1:orig]
  DF = Dindex[1:orig, ]
  # Add in the previous predictors
  # Lag the y-variable
  ar_y1 = c(NA, y[-length(y)])
  lag_y1 = ar_y1[1:orig]
  # lag_y1 = c(NA, y1[-length(y1)])
  # Will come back to the MA part
  # Find the empirical average of the VaR up until the data
  # emp_qnt = quantile(y1, tau)
  # Note on 2/13 - this code has a subdivision based on the number of observations. I am making no such division
  # However, this could prove important. Below is from the code originally found online
  # emp_qnt <- if (obs < 300){
    # quantile(data[1:obs], pval)  
  # } else {
    # quantile(data[1:300], pval)
  # }
  # Now we must include the incorporation for the different types of VaR. 
  # We'll use the same labeling as before.
  # model type (1 - SAV, 2 - AS, 3 - GARCH, 4 - ADAPTIVE) (defaults to 1)
  # 1 - SAV
  # if (model == 1){
    # DF_mod = cbind(pc_df, emp_qnt)
  # }
  # Append the first lag to the data frame
  DF_wl = cbind(DF,lag_y1)
  ...
  # mm = lm(y1 ~ DF) - old function
  # mm = rq(y1 ~ DF, tau = tau)
  # Here's the new function with an untransformed AR(1) lag
  if (ar_tf == 1){
    mm = rq(y1[-(1:p)] ~ DF_trim, tau = tau)
  }
  # Here's the new function with an SAV AR(1) lag
  # Identify the lagged term
  ar = ncol(DF_trim)
  if (ar_tf == 2){
    mm = rq(y1[-(1:p)] ~ DF_trim[,-ar] + abs(DF_trim[,ar]), tau = tau)
  }
  ...
  if (ar_tf == 3){
    mm = rq(y1[-(1:p)] ~ DF_trim[,-ar] + DF_trim[,ar] + indi, tau = tau)
    # Add a different line to account for the indicator variable
    coef = matrix(mm$coefficients, (m + 3), 1)
  }
  if (print == 1){
    print(summary(mm))
  }
  ...
  # coef = matrix(mm$coefficients, (m + 1), 1)
  if (ar_tf != 3){
    coef = matrix(mm$coefficients, (m + 2), 1)
  }
  # Initializes yhat variables and MSE
  yhat = NULL
  MSE = NULL
  if (orig < nT) {
    ...
    newx = cbind(rep(1, (nT - orig)), Dindex[(orig + 1):nT, ], ar_y1[(orig+1):nT])
    # Incorporate lagged variables
    if (ar_tf == 3){
      newx = cbind(rep(1, (nT - orig)), Dindex[(orig + 1):nT, ], ar_y1[(orig+1):nT], ifelse(ar_y1[(orig+1):nT] < 0,0,1))
    }
    # [nfcstx(m+1)]*[(m+1)x1] = [nfcstx1]
    yhat = newx %*% coef
    # Calculates errors
...
var_1pc_ar1 = mod_di_wl(pc_df[,1], pc_df[,-1], 100, 2, 0.01, ar_tf = 1, print = 1)
var_1pc_ar2 = mod_di_wl(pc_df[,1], pc_df[,-1], 100, 2, 0.01, ar_tf = 2, print = 1)
var_1pc_ar3 = mod_di_wl(pc_df[,1], pc_df[,-1], 100, 2, 0.01, ar_tf = 3, print = 1)
```
...
loss_calc = function(y, x, orig, end, m, tau, mod_di = 0, ar_tf = 1, p = 1, print = 0, model = 1){
  # Extract y_hat values
  if (mod_di == 0){
    di = mod_di(y,x,orig,m, tau)
  }
  else {
    di = mod_di_wl(y,x,orig,m, tau, ar_tf = ar_tf, p = p, print = print, model = model)
  }
  # mod_di_wl = function (y, x, orig, m, tau, ar_tf = 1, p = 1, print = 0, model = 1)
...
test = loss_calc(pc_df[,1], pc_df[,-1], 757, 1027, 1, 0.01, mod_di = 1, ar_tf = 1)
# test
...
opt_m = function(y, x, orig, end, tau, low_m = 1, high_m, mod_di = 0, ar_tf = 1, p = 1, print = 0, model = 1){
  ...
  for (i in 1:length(loss_vec)){
    loss_vec[i] = loss_calc(y=y,x=x,orig=orig,end=end, m = m_vec[i], tau = tau, mod_di = mod_di, ar_tf = ar_tf, p = p, print = print, model = model)[[1]]
  }
  # Find the minimizer
...
# uvcav_1
plot_mat = cbind(pc_df[1008:1257,1], mv_fcst$yhat[1:250], mv_fcst_art1$yhat[1:250], mv_fcst_art2$yhat[1:250], mv_fcst_art3$yhat[1:250], uvcav_1[1008:1257], uvcav_2[1008:1257], uvcav_3[1008:1257], uvcav_4[1008:1257])
```
...
#'
#' @examples = plt_data(plot_mtx[[1]])
plt_data = function(plot_matrix, norm_value = 100){
  # Establish a maximum and minimum value
  max_val = max(plot_matrix[,2:ncol(plot_matrix)]) - 100
  min_val = min(plot_matrix[,2:ncol(plot_matrix)]) - 100
  # Create an initial plot and add lines
    for (i in 2:ncol(plot_matrix)){
      if (i == 2){
        plot(as.Date(plot_matrix[,1]), plot_matrix[,i]-norm_value, type = "l", xlab = "Date", ylab = "Ending Percentage Change", ylim = c(min_val,max_val), lwd = 4, main = "Cumulative Forecast Return (Excluding Dividends) by D.I. Number")
    } else if(i %in% seq(2,8,1)) {
        lines(as.Date(plot_matrix[,1]), plot_matrix[,i]-norm_value, col = i-1, lty = 2)
    } else {
        lines(as.Date(plot_matrix[,1]), plot_matrix[,i]-norm_value, col = i-1, lty = 3, lwd = 2)
    }
    }
  # Define a sequence for plotting
  plot_seq = seq(1, ncol(plot_matrix)-1)
  legend("topleft", legend = c("True Val", (plot_seq[2:length(plot_seq)] -1)), col = plot_seq, lty = c(1, rep(2, 7), rep(3, ifelse(ncol(plot_matrix)-8 <= 0, 0, ncol(plot_matrix)-8))), lwd = c(4, rep(1, 7), rep(2, ifelse(ncol(plot_matrix)-8 <= 0, 0, ncol(plot_matrix)-8))))
  # legend("topleft", legend = plot_seq, col = plot_seq, lty = c(1, rep(2, 7), rep(3, ncol(plot_matrix)-8)), lwd = c(4, rep(1, 7), rep(2, ncol(plot_matrix)-8)))
  # Add a line for 0
...
# function(symbol_list, resp_var = "SPY", compl_case = 1, adj_close = 1, resp_adj_close = 1, start_date = "1900-01-01", end_date = Sys.Date())
# df = diff_index_df(c("DIS", "GE", "IBM", "MMM", "XOM"), resp_var = "PG", start_date = "2004-01-01", end_date = "2008-12-31")
# pc_df = pc_diff_index(df)

cav_simul = function(symbol_list, resp_var, compl_case = 1, adj_close = 1, resp_adj_close = 1, start_date = "1900-01-01", end_date = Sys.Date(), orig, end, tau, low_m = 1, high_m, ){
  ...
  opt_pred_nl = opt_m(y = pc_df[,1], x = pc_df[,-1], orig = orig, end = end, tau = tau, low_m = low_m, high_m = high_m)
  opt_pred_art1 = opt_m(y = pc_df[,1], x = pc_df[,-1], orig = orig, end = end, tau = tau, low_m = low_m, high_m = high_m, mod_di = 1, ar_tf = 1)
  opt_pred_art2 = opt_m(y = pc_df[,1], x = pc_df[,-1], orig = orig, end = end, tau = tau, low_m = low_m, high_m = high_m, mod_di = 1, ar_tf = 2)
  opt_pred_art3 = opt_m(y = pc_df[,1], x = pc_df[,-1], orig = orig, end = end, tau = tau, low_m = low_m, high_m = high_m, mod_di = 1, ar_tf = 3)
  # Run the rq functions for the optimal number of parameters
  mv_fcst = mod_di(pc_df[,1], pc_df[,-1], 1007, 3, 0.01)
  mv_fcst_art1 = mod_di_wl(pc_df[,1], pc_df[,-1], 1007, 3, 0.01, ar_tf = 1)
  mv_fcst_art2 = mod_di_wl(pc_df[,1], pc_df[,-1], 1007, 3, 0.01, ar_tf = 2)
  mv_fcst_art3 = mod_di_wl(pc_df[,1], pc_df[,-1], 1007, 3, 0.01, ar_tf = 3)
  # Run the mod_di function and mod_di_wl function and get predictions
  # Run the rolling predictions function
  # Plot everything
  # function(y, x, orig, end, tau, low_m = 1, high_m, mod_di = 0, ar_tf = 1, p = 1, print = 0, model = 1){
  # opt_pred_nl = opt_m(y = pc_df[,1], x = pc_df[,-1], orig = 757, end = 1007, tau = 0.01, low_m =1, high_m  = 5)
  # opt_pred_art1 = opt_m(y = pc_df[,1], x = pc_df[,-1], orig = 757, end = 1007, tau = 0.01, low_m =1, high_m = 5, mod_di = 1, ar_tf = 1)
  # opt_pred_art2 = opt_m(y = pc_df[,1], x = pc_df[,-1], orig = 757, end = 1007, tau = 0.01, low_m =1, high_m = 5, mod_di = 1, ar_tf = 1)
  # opt_pred_art3 = opt_m(y = pc_df[,1], x = pc_df[,-1], orig = 757, end = 1007, tau = 0.01, low_m =1, high_m = 5, mod_di = 1, ar_tf = 1)
