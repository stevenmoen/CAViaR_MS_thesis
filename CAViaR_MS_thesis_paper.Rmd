...
var_1pc_ar1 = mod_di_wl(pc_df[,1], pc_df[,-1], 100, m = 3, 0.01, ar_tf = 1, print = 1, p =1)
var_1pc_ar2 = mod_di_wl(pc_df[,1], pc_df[,-1], 100, m = 2, 0.01, ar_tf = 2, print = 1, p =4)
var_1pc_ar3 = mod_di_wl(pc_df[,1], pc_df[,-1], 100, m = 2, 0.01, ar_tf = 3, print = 1, p =2)
...
#'
#' @examples - opt_m(pc_df[,1], pc_df[,-1], 757, 1027, 0.01, low_m =1, high_m  = 5)
opt_p = function(y, x, orig, end = NULL, tau, low_m = 1, high_m, low_p = 1, high_p, mod_di = 0, ar_tf = 1, print = 0, model = 1){
  ...
  for (i in 1:nrow(loss_mat)){
    loss_mat[i,] = opt_m(y = y, x = x, orig = orig, end = end, tau = tau, low_m = low_m, high_m  = high_m, p = i, mod_di = mod_di, ar_tf = ar_tf, print = print, model = model)[[2]]
  }
  # Find the minimizer
  # print(which(x == min(x), arr.ind = TRUE))
  opt_p = which(loss_mat == min(loss_mat), arr.ind = TRUE)[1,1]
  opt_m = which(loss_mat == min(loss_mat), arr.ind = TRUE)[1,2]
  # Return the loss_vector and the minimzer
  return(list(opt_p, opt_m, loss_mat))
}
```

```{r}
blah = opt_p(y = pc_df[,1], x = pc_df[,-1], orig = 757, end = 1007, tau = 0.01, low_m =1, high_m  = 5, low_p = 1, high_p = 10, ar_tf = 2, mod_di = 1)
# ?persp3D
