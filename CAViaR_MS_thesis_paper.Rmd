...
mod_di_wl = function (y, x, orig, m, tau, ar_tf = 1, p = 1, print = 0, model = 1, end = NULL) 
{
  ...
  for (i in 1:p){
    ...
    lag_var = c(rep(NA, p), y[-(p*length(y))])
    # Truncate the length
    lag_var = lag_var[1:orig]
  ...
  # Identify the first and last AR terms
  # l_ar = ncol(DF_trim)
  # f_ar = l_ar - p + 1
  if (ar_tf == 2){
    mm = rq(y1[-(1:p)] ~ DF_trim[,-(f_ar:l_ar)] + abs(DF_trim[,(f_ar:l_ar)]), tau = tau)
  }
  ...
  for (i in 1:p){
    ...
    indi_mat = matrix(0, nrow(DF_trim), p)
    # Populate the indicator
    indi_mat[,p] = ifelse(DF_trim[,f_ar + p - 1] < 0, 0, 1)
  }
  # Fitting the regression
  # summary(fit <- lm(pgas~ poil + poilL + indi, data=mess))
  if (ar_tf == 3){
    mm = rq(y1[-(1:p)] ~ DF_trim[,-(f_ar:l_ar)] + DF_trim[,(f_ar:l_ar)] + indi_mat, tau = tau)
  ...
  if (orig < nT) {
    ...
    if (ar_tf == 3){
      newx = cbind(rep(1, (nT - orig)), Dindex[(orig + 1):nT, ], all_lag[(orig+1):nT], ifelse(all_lag[(orig+1):nT] < 0,0,1))
    }
    # [nfcstx(m+1)]*[(m+1)x1] = [nfcstx1]
    # print(dim(newx))
    # print(dim(coef))
...
var_1pc_ar1 = mod_di_wl(pc_df[,1], pc_df[,-1], 100, 2, 0.01, ar_tf = 1, print = 1, p =2)
var_1pc_ar2 = mod_di_wl(pc_df[,1], pc_df[,-1], 100, 2, 0.01, ar_tf = 2, print = 1, p =2)
var_1pc_ar3 = mod_di_wl(pc_df[,1], pc_df[,-1], 100, 2, 0.01, ar_tf = 3, print = 1, p =2)
```
