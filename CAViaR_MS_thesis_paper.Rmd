...
opt_m(pc_df[,1], pc_df[,-1], 757, 1027, 0.01, low_m =1, high_m  = 5)
...
```{r}
# Run the model
# mv_fcst = mod_di(pc_df[,1], pc_df[,-1], 1027, 3, 0.01)

# length(pc_df[,1])

# Run the univariate CAViaR models
...
```{r}
# Create a plot
ts.plot(pc_df[1028:1257,1], ylim = c(-0.1,0.1), xlab = "Trading Days", ylab = "Percent Change in PG")
lines(mv_fcst$yhat[1:250], col = "blue", lty = 2)
lines(as.ts(uvcav_1[1028:1257])*(-1), col = "red", lty = 2)
lines(as.ts(uvcav_2[1028:1257])*(-1), col = "green", lty = 2)
lines(as.ts(uvcav_3[1028:1257])*(-1), col = "orange", lty = 2)
lines(as.ts(uvcav_4[1028:1257])*(-1), col = "purple", lty = 2)
# length(uvcav_1)
```

```{r}
# Write a function to calculate losses on the test sample
...
```{r}
# Calculate the losses
loss_mv = loss_test(pc_df[1028:1257,1], mv_fcst$yhat[1:250], tau = 0.01)[[1]]
loss_uv1 = loss_test(pc_df[1028:1257,1], as.ts(uvcav_1[1028:1257])*(-1), tau = 0.01)[[1]]

print(c(loss_mv, loss_uv1))

# Calculate the VaR breaks
break_mv = loss_test(pc_df[1028:1257,1], mv_fcst$yhat[1:250], tau = 0.01)[[3]]
break_uv1 = loss_test(pc_df[1028:1257,1], as.ts(uvcav_1[1028:1257])*(-1), tau = 0.01)[[3]]

print(c(break_mv, break_uv1))
