...
#'
#' @examples
mod_di = function (y, x, orig, m, tau, end = NULL) 
{
...
loss_calc = function(y, x, orig, m, tau, mod_di = 0, ar_tf = 1, p = 1, print = 0, model = 1, end = NULL){
  ...
  if (mod_di == 0){
    di = mod_di(y=y,x=x,orig=orig,m=m, tau=tau, end = end)
  }
  else {
...
cav_simul = function(symbol_list, resp_var, compl_case = 1, adj_close = 1, resp_adj_close = 1, start_date = "1900-01-01", end_date = Sys.Date(), nval, ntest, tau, low_m = 1, high_m, uv_list = NULL, no_run = c(0,0,0,0)){
  ...
  mv_fcst_art3 = mod_di_wl(pc_df[,1], pc_df[,-1], orig = test_orig, m = opt_pred_art3[[1]], tau = tau, ar_tf = 3)
...
```{r}
var_1pc_2008_sm = cav_simul(c("DIS", "GE", "IBM", "MMM", "XOM"), resp_var = "PG", start_date = "2004-01-01", end_date = "2008-12-31", nval = 250, ntest = 20, low_m = 1, high_m = 5, tau = 0.01)
