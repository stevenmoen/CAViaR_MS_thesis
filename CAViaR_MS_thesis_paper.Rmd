...
mod_di_wl = function (y, x, orig, m, tau, ar_tf = 1, p = 1, print = 0, model = 1, end = NULL) 
{
  ...
  for (i in 1:p){
    ...
    lag_var = lag(y, i)
    # lag_var = c(rep(NA, p), y[-((length(y) - p + 1):length(y))])
    # print(head(lag_var))
    # Truncate the length
  ...
  if (ar_tf == 1){
    ...
    print(mm)
    # Check on 3/30/20 - what happens if we don't exclude the rows but try na.omit
    print(dim(y1))
    print(dim(DF_wl))
    print(head(y1))
    print(head(DF_wl))
    mm_na = rq(y1 ~ DF_wl, tau = tau, na.action = na.exclude)
    # print(mm_na)
  }
  ...
  if (ar_tf == 3){
    ...
    for (i in 1:p){
      ...
      indi_mat[,i] = ifelse(DF_trim[,f_ar + i - 1] < 0, 0, 1)
    }
  }
  # print(head(indi_mat))
  ...
  yhat = NULL
  MSE = NULL
  # Print the dimensions of the data frame and nT
  print(dim(DF_trim))
  print(nT)
  print(dim(Dindex))
  print(dim(all_lag))
...
```{r}
# mod_di_wl = function (y, x, orig, m, tau, ar_tf = 1, p = 1, print = 0, model = 1, end = NULL) 
mv_fcst_art3 = mod_di_wl(y = pc_df[,1], x = pc_df[,-1], orig = 1007, m = 5,  tau = 0.01, ar_tf = 1, p = 1)
# print(head(pg))
names(pc_df)[-1]
```
...
var_1pc_ar1 = mod_di_wl(pc_df[,1], pc_df[,-1], 100, 3, 0.01, ar_tf = 1, print = 1, p =1)
