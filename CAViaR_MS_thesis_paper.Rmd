...
#' @param print - print the model summary and the MSE
#'
#' @return - returns a list of variables for use in the diffusion index
#' @export
#'
#' @examples
mod_di = function (y, x, orig, m, tau, end = NULL, print = 0) 
{
...
```{r}
# mv_fcst_art3 = mod_di_wl(pc_df[,1], pc_df[,-1], orig = 1007, end = 1108, 3, 0.01, ar_tf = 3)
```
...
```{r}
var_1pc = mod_di(pc_df[,1], pc_df[,-1], 100, 2, 0.01, print = 0)
...
loss_calc = function(y, x, orig, m, tau, mod_di = 0, ar_tf = 1, p = 1, print = 0, model = 1, end = NULL){
  # Extract y_hat values
  if (mod_di == 0){
    di = mod_di(y=y,x=x,orig=orig,m=m, tau=tau, end = end, print = print)
  }
  else {
    di = mod_di_wl(y=y,x=x,orig=orig,m=m, tau=tau, ar_tf = ar_tf, p = p, print = print, model = model, end = end)
  }
...
test = loss_calc(pc_df[,1], pc_df[,-1], orig = 757, end = 1007, 1, 0.01, mod_di = 1, ar_tf = 1)
# test
...
opt_m = function(y, x, orig, end = NULL, tau, low_m = 1, high_m, mod_di = 0, ar_tf = 1, p = 1, print = 0, model = 1){
  ...
  for (i in 1:length(loss_vec)){
    loss_vec[i] = quiet(loss_calc(y=y,x=x,orig=orig,end=end, m = m_vec[i], tau = tau, mod_di = mod_di, ar_tf = ar_tf, p = p, print = print, model = model))[[1]]
  }
  # Find the minimizer
...
plt_data = function(plot_matrix, tau){
...
# as.Date(plot_mat[,1])
abc = plt_data(plot_mat, tau = 0.01)
...
```{r}
pretty_tables = function(data_mat, loss_list, tau){
  # Combine into a data frame
  ...
  rownames(df) <- c("Losses", "VaR Breaks (%)")
  # Convert to a table
  df %>% kable(caption = paste("Comparison of VaR Methods for a ", tau*100, "% VaR", sep = ""), digits = 3) %>% kable_styling("striped", full_width = F) %>% kable_styling()
}
```

```{r}
pretty_tables(plot_mat, l_list, tau = 0.01)
```
...
cav_simul = function(symbol_list, resp_var, compl_case = 1, adj_close = 1, resp_adj_close = 1, start_date = "1900-01-01", end_date = Sys.Date(), nval, ntest, tau, low_m = 1, high_m, uv_list = NULL, no_run = c(0,0,0,0), na_interp = TRUE, print = 0){
...
```{r}
var_1pc_2008_sm = cav_simul(c("DIS", "GE", "IBM", "MMM", "XOM"), resp_var = "PG", start_date = "2004-01-01", end_date = "2008-12-31", nval = 250, ntest = 3, low_m = 1, high_m = 5, tau = 0.01)
